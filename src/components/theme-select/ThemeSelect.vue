<template>
    <div v-if="mobile" class="theme_select" @click="mobileThemeClick">
        ...
        <selector-dialog v-model:is-show="is_show" @optionTap="optionTap" :list="themeNames" :active="theme_selected"></selector-dialog>
    </div>
    <div v-else class="theme_select" :style="{ width: width+'px' }" @mouseenter="select_open" @mouseleave="select_close">
        ...
        <div ref="options_el" class="theme_options" :style="styles">
            <theme-container :background="StyleAttrs.sideBg">
                <div class="option_item" :class="{ active: theme_selected === key }" v-for="item, key in themeNames" :key="'theme_'+key" @click="option_tap(key)">{{ item }}</div>
            </theme-container>
        </div>
    </div>
</template>
...
import ThemeContainer, { StyleAttrs } from '../container/theme-container/ThemeContainer.vue'
const SelectorDialog = defineAsyncComponent(() => import('@/components/selector-dialog/SelectorDialog.vue'))
interface Styles {
    height: string;
    visibility?: string;
    borderWidth?: string;
