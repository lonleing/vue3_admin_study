...
const { apiDir } = require('../src/config/default/mock.config')
const path = require('path')
const fs = require('fs')

const addRouter = (router: any) => {
    function ergodicFile(dir: string, isApisDir=false) {
        fs.readdirSync(dir).forEach((controllerItemPath: string) => {
            ...
            if (isDir) {
                ergodicFile(controllerItemPathFull, apiDir.includes(controllerItemPath))
                ...
                if (routeMap) {
                    ...
                    routeMap.forEach((route) => {
                        ...
                        console.log(apiDir.indexOf(controllerItemPath))
                        // newPath = isApisDir ? '/api' : newPath
